...
sequenceDiagram
    ...
    alt Feature Dev (Full Stack)
        ...
        Dev->>Script: scripts/start-dev-docker.ps1
        Script->>Docker: Start Backend (Docker)
        Script->>Client: Start Vite (Localhost)
        Client->>Docker: Connects to Localhost:9980
    end
```
...
**How to run:**
1.  Run `scripts/start-dev-docker.ps1` (or `start-dev-docker.sh` on macOS/Linux).
2.  Wait for Docker to start (Backend at `localhost:9980`).
3.  Client will open at `localhost:9981`.
4.  Editor will open at `localhost:9982`.

**Note:** This mode uses `.env.docker-web` to configure your frontend to talk to `localhost:9980` instead of `10.0.2.2` (Android).
